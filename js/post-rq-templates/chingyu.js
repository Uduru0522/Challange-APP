...
function appendrooms(){//show rooms in chatroom page
    ...
    for(let i=0;i<room_magnitude;i++){
        ...
        let chatroom="<div id='chat-room-num"+i+"'class='chat-room'><img id='chat-header'src='"+rooms_data[i].header_pic+"'/><div class='chat-room-text'><h3 id='chat-group-name'>"+rooms_data[i].name+"</h3><h4 id='chat-firstline'>"+rooms_data[i].talk+"</h4></div></div>";
...
function appendfriends(){//show friends in group create
    document.getElementById("chat-choose-friends").innerHTML="<h2>選擇好友</h2><form name='group-choose-friend' id='group-choose-friend'><input type='text' name='friendtosearch' id='friendtosearch'><button type='submit' class='search-friend'><img src='../resources/nav/search.png'/></button></form>";
    for(let i=0;i<friend_magnitude;i++){
        ...
        let friends="<input type='checkbox' name='choose_friend' id='C_F"+i+"'><label for='C_F"+i+"'><div id='choosed-friend"+i+"'class='choosed-friend unchosen'><img src='"+friend_list[i].image+"'/><h3>"+friend_list[i].name+"</h3></div>";        
...
$(document).on("click",'.chat-room',function(){
	...
	roomstyle="group";
	roomID=rooms_data[i].group_name;
	if(rooms_data[i])
        $.post('./chatroom_mission', {//****************************************************************
